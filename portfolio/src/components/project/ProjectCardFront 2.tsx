...
import { toast } from "sonner"; // ⬅️ Ici on importe Sonner
...
export default function ProjectCardFront({ projectData }: ProjectCardProps) {
  ...
  const { mutate: handleLike, isPending } = useMutation({
    ...
    onSuccess: (data) => {
      toast.success(data.message || "Projet liké avec succès !"); // ✅ Succès
      queryClient.invalidateQueries({ queryKey: ["projects"] });
    },
    onError: (error: any) => {
      toast.error(error.message || "Erreur lors du like"); // ✅ Erreur
    },
  });
