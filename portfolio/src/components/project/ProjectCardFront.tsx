...
export default function ProjectCardFront({ projectData }: ProjectCardProps) {
  ...
  const { mutate: handleLike, isPending: isLiking } = useMutation({
    mutationFn: async () => {
      ...
      if (!res.ok) throw new Error(data.message || "Erreur lors du like");
      return data;
    },
    onSuccess: (data) => {
      ...
      <Card className="flex flex-col justify-between h-full transition-transform duration-300 hover:-translate-y-2 hover:shadow-lg">
        ...
        <CardContent>
          ...
          <div className="flex flex-wrap gap-2">
            {projectData.list_technos?.map((techno, idx) => (
        ...
        <CardFooter className="mt-auto flex justify-between items-center">
          ...
          <button
              onClick={() => handleLike()}
              disabled={isLiking || hasLikedLocally}
              className={`flex items-center gap-1 transition-colors ${
                  hasLikedLocally ? "text-red-600" : "text-gray-400 hover:text-red-600"
              } disabled:opacity-50`}
          >
            <Heart
                className="w-5 h-5"
                fill={hasLikedLocally ? "currentColor" : "none"}
            />
